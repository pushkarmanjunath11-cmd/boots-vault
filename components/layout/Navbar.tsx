  ...
  return (
    <nav style={{
      ...
      <div style={{ display:'flex', alignItems:'center', gap:4 }}>
        ...
        <button onClick={toggleCart} style={{ width:44, height:44, background:'transparent', border:'none', cursor:'pointer', position:'relative', color:'rgba(242,242,237,0.6)', display:'flex', alignItems:'center', justifyContent:'center', transition:'color 0.2s', transform: cartBounce ? 'scale(1.3)' : 'scale(1)' }}
          onMouseEnter={e => (e.currentTarget.style.color='var(--white)')}
          onMouseLeave={e => (e.currentTarget.style.color='rgba(242,242,237,0.6)')}>
          <ShoppingBag size={20} />
          {count > 0 && (
            <span style={{ position:'absolute', top:6, right:6, width:16, height:16, background:'var(--green)', borderRadius:'50%', fontSize:9, fontWeight:800, color:'#050505', display:'flex', alignItems:'center', justifyContent:'center' }}>{count}</span>
          )}
        </button>
          ...
          <>
            {!user ? (
              <Link href="/login" style={{ textDecoration:'none' }}>
                <div style={{ display:'flex', alignItems:'center', gap:7, padding:'7px 14px', border:'1px solid rgba(242,242,237,0.14)', fontSize:11, fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', color:'rgba(242,242,237,0.6)', cursor:'pointer', transition:'all 0.25s', fontFamily:'Montserrat' }}
              ...
              <div ref={dropRef} style={{ position:'relative' }}>
                <button onClick={() => setDropdown(d => !d)} style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 12px', background: dropdown ? 'rgba(34,197,94,0.1)' : 'rgba(242,242,237,0.04)', border:`1px solid ${dropdown ? 'rgba(34,197,94,0.3)' : 'rgba(242,242,237,0.08)'}`, cursor:'pointer', color:'var(--white)', transition:'all 0.2s', fontFamily:'Montserrat' }}>
                  {user.photoURL ? (
                  ...
                  <span style={{ fontSize:12, fontWeight:700, color:'rgba(242,242,237,0.8)', display:'none' }} className="desktop-only">{firstName}</span>
                  <ChevronDown size={12} color="rgba(242,242,237,0.4)" style={{ transform: dropdown ? 'rotate(180deg)' : 'none', transition:'transform 0.2s' }} />
                </button>

                {dropdown && (
                  <div style={{ position:'absolute', top:'calc(100% + 8px)', right:0, width:200, background:'#0f0f0f', border:'1px solid rgba(242,242,237,0.08)', boxShadow:'0 20px 60px rgba(0,0,0,0.6)', zIndex:200 }}>
