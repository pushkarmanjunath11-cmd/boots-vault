    ...
    <div style={{ background:'var(--bg)', minHeight:'100vh', paddingTop:60, fontFamily:'Montserrat,sans-serif' }}>
      ...
      <div style={{ background:'var(--bg2)', borderBottom:'1px solid var(--border)', padding:'clamp(24px,4vw,40px) clamp(20px,4vw,48px) 0' }}>
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          ...
          <h1 className="font-display" style={{ fontSize:'clamp(40px,7vw,88px)', color:'var(--white)', lineHeight:0.9, marginBottom:28 }}>
          ...
          <div style={{ display:'flex', gap:0, overflowX:'auto', WebkitOverflowScrolling:'touch', scrollbarWidth:'none' }}>
            {cats.map(c => (
              <button key={c.id} onClick={() => setCategory(c.id)} style={{ padding:'12px clamp(12px,2vw,24px)', fontSize:11, fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', background:'transparent', color: category===c.id ? 'var(--white)' : 'rgba(242,242,237,0.3)', border:'none', borderBottom: category===c.id ? '2px solid var(--green)' : '2px solid transparent', cursor:'pointer', transition:'all 0.2s', fontFamily:'Montserrat', whiteSpace:'nowrap' }}>
                {c.label}
              </button>
            ))}
          </div>
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'clamp(16px,3vw,32px) clamp(20px,4vw,48px)' }}>
