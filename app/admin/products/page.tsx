...
import { Trash2, Edit3, ChevronLeft, ChevronRight, Plus, Check, X, Loader2, Search } from 'lucide-react'

const catLabel: Record<string, string> = {
  'boots': 'Boots', 'jerseys-jackets': 'Jerseys', 'balls': 'Balls',
  'gloves': 'Gloves', 'essentials': 'Essentials',
  'football-boots': 'FG/AG', 'turf': 'Turf', 'futsal': 'Futsal',
}

const catColor: Record<string, string> = {
  'boots': '#22c55e', 'jerseys-jackets': '#60a5fa', 'balls': '#f97316',
  'gloves': '#c084fc', 'essentials': '#d4af37',
  'football-boots': '#22c55e', 'turf': '#60a5fa', 'futsal': '#f97316',
}
    ...
    <div style={{ color: '#f5f5f0', fontFamily: 'Montserrat, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28, flexWrap: 'wrap', gap: 16 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: 'rgba(245,245,240,0.3)', marginTop: 4 }}>
            {search ? `${filtered.length} of ${products.length}` : `${products.length}`} products
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          ...
          <div style={{ position: 'relative' }}>
            <Search size={13} color="rgba(245,245,240,0.25)" style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }} />
            <input
              value={search}
              onChange={e => setSearch(e.target.value)}
              placeholder="Search products..."
              style={{ background: '#0d0d0d', border: '1px solid rgba(245,245,240,0.07)', padding: '9px 14px 9px 32px', fontSize: 12, color: '#f5f5f0', outline: 'none', width: 210, fontFamily: 'Montserrat', transition: 'border-color 0.2s' }}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: 80 }}>
          <p className="font-display" style={{ fontSize: 32, color: 'rgba(245,245,240,0.06)', marginBottom: 12 }}>
            {search ? 'NO RESULTS FOUND' : 'NO PRODUCTS YET'}
          </p>
          {!search && (
            <Link href="/admin/add-product" style={{ textDecoration: 'none' }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '12px 24px', background: '#22c55e', color: '#050505', fontSize: 12, fontWeight: 800, cursor: 'pointer', fontFamily: 'Montserrat' }}>
                <Plus size={14} /> Add Your First Product
              </div>
            </Link>
          )}
          {search && (
            <button onClick={() => setSearch('')} style={{ fontSize: 12, color: 'rgba(245,245,240,0.3)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'Montserrat', marginTop: 8 }}>
              Clear search
            </button>
          )}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {filtered.map(p => {
            ...
            return (
              <div key={p.id} style={{ background: '#0d0d0d', border: '1px solid rgba(245,245,240,0.05)', borderLeft: `3px solid ${col}`, position: 'relative' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '180px 1fr auto', gap: 0 }}>
                  ...
                  <div style={{ position: 'relative', height: 140, background: '#0a0a0a', overflow: 'hidden' }}>
                    {p.images?.length > 0 ? (
                      <>
                        <img src={p.images[curImg]} alt={p.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                          onError={e => (e.currentTarget.style.display = 'none')} />
                        {p.images.length > 1 && (
                          <>
                            <button onClick={() => prevImg(p.id, p.images.length)} style={{ position: 'absolute', left: 4, top: '50%', transform: 'translateY(-50%)', width: 24, height: 24, background: 'rgba(5,5,5,0.8)', border: 'none', cursor: 'pointer', color: '#f5f5f0', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                              <ChevronLeft size={12} />
                            </button>
                            <button onClick={() => nextImg(p.id, p.images.length)} style={{ position: 'absolute', right: 4, top: '50%', transform: 'translateY(-50%)', width: 24, height: 24, background: 'rgba(5,5,5,0.8)', border: 'none', cursor: 'pointer', color: '#f5f5f0', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                              <ChevronRight size={12} />
                            </button>
                            <div style={{ position: 'absolute', bottom: 6, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 3 }}>
                              {p.images.map((_, i) => (
                                <div key={i} onClick={() => setImgIndex(prev => ({ ...prev, [p.id]: i }))} style={{ width: i === curImg ? 12 : 5, height: 5, borderRadius: 2.5, background: i === curImg ? '#22c55e' : 'rgba(255,255,255,0.3)', cursor: 'pointer', transition: 'all 0.2s' }} />
                              ))}
                            </div>
                  ...
                  <div style={{ padding: '16px 20px' }}>
                    {isEditing ? (
                        ...
                        <div style={{ display: 'flex', gap: 16 }}>
                          {[['inStock', 'In Stock'], ['featured', 'Featured'], ['isNew', 'New']].map(([k, lbl]) => (
                            <label key={k} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontSize: 11, color: 'rgba(245,245,240,0.5)' }}>
                              <div onClick={() => setEditForm(p => ({ ...p, [k]: !p[k as keyof typeof p] }))} style={{ width: 14, height: 14, border: `1px solid ${editForm[k as keyof typeof editForm] ? '#22c55e' : 'rgba(245,245,240,0.15)'}`, background: editForm[k as keyof typeof editForm] ? '#22c55e' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                      ...
                      <>
                        ...
                        <p style={{ fontSize: 15, fontWeight: 700, color: '#f5f5f0', marginBottom: 8 }}>{p.name}</p>
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                          <span className="font-display" style={{ fontSize: 24, color: '#f5f5f0' }}>₹{p.price.toLocaleString()}</span>
                        ...
                        <p style={{ fontSize: 10, color: 'rgba(245,245,240,0.2)', marginTop: 6 }}>
                          Sizes: {Array.isArray(p.sizes) ? p.sizes.join(', ') : '—'} · {p.images?.length ?? 0} image{p.images?.length !== 1 ? 's' : ''}
                        </p>
                      </>
                    )}
                  </div>
                  ...
                  {/* Actions */}
                  <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: 8, justifyContent: 'center', borderLeft: '1px solid rgba(245,245,240,0.04)' }}>
                    {isEditing ? (
                      <>
                        <button onClick={() => saveEdit(p.id)} disabled={saving} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: '#22c55e', color: '#050505', border: 'none', fontSize: 10, fontWeight: 800, letterSpacing: '0.15em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'Montserrat', whiteSpace: 'nowrap' }}>
                          <Check size={11} /> {saving ? 'Saving...' : 'Save'}
                        </button>
                        <button onClick={cancelEdit} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: 'transparent', color: 'rgba(245,245,240,0.3)', border: '1px solid rgba(245,245,240,0.08)', fontSize: 10, fontWeight: 700, cursor: 'pointer', fontFamily: 'Montserrat', whiteSpace: 'nowrap' }}>
                          <X size={11} /> Cancel
                        </button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => startEdit(p)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: 'rgba(245,245,240,0.04)', color: 'rgba(245,245,240,0.5)', border: '1px solid rgba(245,245,240,0.07)', fontSize: 10, fontWeight: 700, cursor: 'pointer', fontFamily: 'Montserrat', whiteSpace: 'nowrap', transition: 'all 0.2s' }}
                        ...
                        <button onClick={() => handleDelete(p.id, p.name)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: 'rgba(248,113,113,0.06)', color: 'rgba(248,113,113,0.4)', border: '1px solid rgba(248,113,113,0.1)', fontSize: 10, fontWeight: 700, cursor: 'pointer', fontFamily: 'Montserrat', whiteSpace: 'nowrap', transition: 'all 0.2s' }}
