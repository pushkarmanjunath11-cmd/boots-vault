...
export default function ProductPage() {
  ...
  return (
    <div style={{ background:'var(--bg)', minHeight:'100vh', paddingTop:60, fontFamily:'Montserrat,sans-serif' }}>
      ...
      {/* Breadcrumb */}
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'20px clamp(20px,4vw,48px) 0' }}>
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'24px clamp(20px,4vw,48px) 80px' }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(min(100%, 340px), 1fr))', gap:'clamp(24px,4vw,56px)', alignItems:'start' }}>
          ...
          <div style={{ opacity: loaded ? 1 : 0, transform: loaded ? 'none' : 'translateX(-20px)', transition:'all 0.8s cubic-bezier(0.22,1,0.36,1)' }}>
            ...
            <div style={{ position:'relative', background:'linear-gradient(135deg, var(--bg4), var(--bg3))', aspectRatio:'1/1', overflow:'hidden', marginBottom:12, border:'1px solid var(--border)' }}>
              ...
              <div className="font-display" style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', fontSize:200, color:'rgba(242,242,237,0.02)', userSelect:'none', pointerEvents:'none', lineHeight:1 }}>
                {product.brand?.[0] || 'B'}
              </div>
            ...
            {images.length > 1 && (
              <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                {images.map((img, i) => (
                  <button key={i} onClick={() => setImgIdx(i)} style={{ width:68, height:68, position:'relative', border:`2px solid ${i===imgIdx ? 'var(--green)' : 'var(--border)'}`, background:'var(--bg4)', cursor:'pointer', overflow:'hidden', padding:0, transition:'border-color 0.2s', flexShrink:0 }}>
                    <img src={img} alt="" style={{ width:'100%', height:'100%', objectFit:'cover', opacity: i===imgIdx ? 1 : 0.4, transition:'opacity 0.2s' }}
                      onError={e => (e.currentTarget.style.display='none')} />
                    <div className="font-display" style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize:24, color:'rgba(242,242,237,0.06)', userSelect:'none' }}>{product.brand?.[0] || 'B'}</div>
                  </button>
                ))}
              </div>
            )}
          ...
          <div style={{ opacity: loaded ? 1 : 0, transform: loaded ? 'none' : 'translateX(20px)', transition:'all 0.8s cubic-bezier(0.22,1,0.36,1) 0.15s' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginTop:24, paddingTop:24, borderTop:'1px solid var(--border)' }}>
              {[
        ...
        {related.length > 0 && (
          <div style={{ marginTop:80 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:28 }}>
              <h2 className="font-display" style={{ fontSize:'clamp(24px,4vw,40px)', color:'var(--white)' }}>YOU MAY ALSO LIKE</h2>
